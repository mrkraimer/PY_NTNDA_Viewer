...
class ImageDisplay(RawImageWidget,QObject):
    ...
    def setPixelLevels(self,pixelLevels) :
        ...
        try :
           ...
           self.error = "setPixelLevels error=" + repr(error)
           return 1
...
# initialize
    ...
    def pixelLevelsEvent(self) :
        self.event.wait()
        try:
            rtn = self.imageDisplay.setPixelLevels(self.pixelLevelsText.text())
            if rtn != 0 : self.statusText.setText(self.imageDiaplay.getError())
        except Exception as error:
            self.statusText.setText(repr(error))
        self.event.set()
    ...
    def mycallback(self,arg):
        ...
        if codecNameLength == 0 : 
            ...
            if codecName=='blosc' : 
                ...
                return
        datatype = data.dtype
        self.dtypeText.setText(str(datatype))
        nx = dimArray[0]["size"]
        ny = dimArray[1]["size"]
        nz = 1
        if ndim==3 : nz = dimArray[2]["size"]
        self.nxText.setText(str(nx))
        self.nyText.setText(str(ny))
        self.nxText.setText(str(nz))
